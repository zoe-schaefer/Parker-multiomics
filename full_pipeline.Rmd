...
library(pacman)
p_load(
  rmarkdown, clusterProfiler, data.table, DOSE, EnhancedVolcano, enrichplot,
  eulerr, ggvenn, Homo.sapiens, igraph, knitr, lintr, OrganismDbi, paletteer,
  qualpalr, readxl, scales, tidyverse, viridis
)
...
total_KB <- total_KB %>% 
  dplyr::select(c("Accession",
                  dplyr::starts_with(c("Abundance Ratio (log2):",
...
total_KB <- total_KB %>% 
  dplyr::rename("log2(FC)" = dplyr::starts_with("Abundance Ratio (log2)"),
         "P value" = dplyr::starts_with("Abundance Ratio Adj. P-Value")
)
...
Group2 <- c("CD9", "CD82", "NFKB1", "BST2", "CTSH")
...
rm(total_GO_down, total_GO_down_BP, total_GO_down_CC, total_GO_down_MF,
   total_GO_down_slice_BP, total_GO_down_slice_CC, total_GO_down_slice_MF,
   total_GO_up, total_GO_up_BP, total_GO_up_CC, total_GO_up_MF,
   total_GO_up_slice_ALL, total_GO_up_slice_BP, total_GO_up_slice_CC,
   total_GO_up_slice_MF,
   total_KEGG_down, total_KEGG_up, total_KEGG_up_slice)
```
...
phospho_raw$CatShapes <- case_when(
  phospho_raw$SYMBOL2 %in% Group1_phospho ~ "1",
  phospho_raw$SYMBOL2 %in% Group2_phospho ~ "2",
  .default = "20"
)

phospho_raw$CatNames <- case_when(
  phospho_raw$SYMBOL2 %in% Group1_phospho_sub |
    phospho_raw$SYMBOL2 %in% Group2_phospho ~ phospho_raw$SYMBOL2,
  .default = ""
)
```

```{r phospho-grouped-volcano-plot}
ggplot(arrange(phospho_raw, desc(CatShapes)), aes(x = `log2(FC)`, y = -log10(`P value`))) +
  geom_point(aes(color = CatShapes, shape = CatShapes)) +
  geom_vline(xintercept = c(phospho_FC_threshold, -phospho_FC_threshold),
             alpha = 0.5, linetype = "dashed") +
  geom_hline(yintercept = -log10(phospho_p_threshold),
             alpha = 0.5, linetype = "dashed") +
  geom_text_repel(aes(label = CatNames), show.legend = FALSE, max.overlaps = Inf,
                  size = rel(3)) +
  guides(alpha = "none", shape = "none") +
  labs(x = "log2(Fold Change)", y = "-log10(P value)",
...
```{r clear-2}
# Clearing large variables
rm(phospho_GO_down, phospho_GO_down_BP, phospho_GO_down_CC, phospho_GO_down_MF,
   phospho_GO_down_slice_BP, phospho_GO_down_slice_CC, phospho_GO_down_slice_MF,
   phospho_GO_up, phospho_GO_up_BP, phospho_GO_up_CC, phospho_GO_up_MF,
   phospho_GO_up_slice_ALL, phospho_GO_up_slice_BP, phospho_GO_up_slice_CC,
   phospho_GO_up_slice_MF,
   phospho_KEGG_down, phospho_KEGG_down_slice,
   phospho_KEGG_up, phospho_KEGG_up_slice,
   total_significant_up, total_significant_down,
   total_up_bound, total_down_bound,
   phospho_significant_up, phospho_significant_down,
   phospho_up_bound, phospho_down_bound,
   kegg_category)
```
...
```{r clear-3}
# Clearing large variables
rm(histone_frag_raw, histone_single_raw, ratios_frag, ratios_frag_invalid,
   ratios_frag_plot, ratios_p, ratios_single, ratios_single_invalid,
   single_p)
```
...
# 6. Epigenetic network
This section develops a network visualization of all epigenetic interactors as defined by EpiFactors (https://epifactors.autosome.org/). Increased dataset size may make networks difficult to graph, so an alternative visualization is provided that breaks down the key complexes involved.

```{r network-setup}
genes_epi <- fread("~/Documents/R/genes.csv")
total_join <- inner_join(dplyr::select(genes_epi, !c("HGNC approved name", "UniProt ID (human)", "Pfam domains", "UniProt ID (mouse)")), dplyr::select(total_significant, "log2(FC)", "P value", "SYMBOL2"), by = join_by("HGNC approved symbol" == "SYMBOL2")) %>% mutate("Source" = "Total")

phospho_join <- inner_join(dplyr::select(genes_epi, !c("HGNC approved name", "UniProt ID (human)", "Pfam domains", "UniProt ID (mouse)")), dplyr::select(phospho_significant, "log2(FC)", "P value", "SYMBOL2"), by = join_by("HGNC approved symbol" == "SYMBOL2")) %>% mutate("Source" = "Phospho")
...
ggplot(joined_data, aes(x = `log2(FC)`, y = -log10(`P value`))) +
  ...
  geom_hline(yintercept = -log10(total_p_threshold),
             alpha = 0.5, linetype = "dashed") +
  labs(x = "log2(Fold Change)", y = "-log10(P value)",
       title = "Figure 6a: Volcano plot") +
  geom_text_repel(aes(label = joined_data$"HGNC approved symbol"), show.legend = FALSE, size = rel(3)) +
  theme(legend.position = "bottom") +
  scale_fill_viridis_d(
    aesthetics = "color", option = "magma",
    begin = 0.2, end = 0.9
...
# filter by that list
total_histone_int <- filter(ratios_single_plot, grepl(targets, PTM))
...
ggplot(total_histone_int, aes(x = `log2(FC)`, y = -log10(`P value`))) +
  ...
  geom_hline(yintercept = -log10(total_p_threshold),
             alpha = 0.5, linetype = "dashed") +
  labs(x = "log2(Fold Change)", y = "-log10(P value)",
       title = "Figure 6b: Volcano plot") +
  geom_text_repel(aes(label = total_histone_int$PTM), show.legend = FALSE, size = rel(3)) +
  theme(legend.position = "bottom") +
  scale_fill_viridis_d(
    aesthetics = "color", option = "magma",
    begin = 0.2, end = 0.9
...
genes_pivoted <- dplyr::select(genes_epi, c("HGNC approved symbol", "Protein complex", "Target entity", "Product")) %>% dplyr::rename("Symbol" = "HGNC approved symbol", "Complex" = "Protein complex", "Target" = "Target entity", "Product" = "Product")
...
genes_filtered_complex <- filter(genes_pivoted, Complex != "#")
genes_filtered_target <- filter(genes_pivoted, Target != "#")
genes_filtered_product <- filter(genes_pivoted, Product != "#")
...
genes_histone_pivoted <- pivot_longer(genes_pivoted, cols = c("Target", "Product"), names_to = NULL,
             values_to = "PTM")

genes_histone_joined <- inner_join(genes_histone_pivoted,
...
color_opts <- c("#FFFFFF", qualpal(n = (length(complexes) - 1), colorspace = "rainbow")$hex)
...
genes_histone_joined <- full_join(genes_histone_joined, complex_df, by = join_by("Complex" == "complexes"))

complex_vertices <- left_join(complex_vertices, genes_histone_joined, by = join_by("vertices" == "Symbol"), multiple = "first")
complex_vertices[complex_vertices == "#"] <- "No ID"

complex_graph <- graph_from_data_frame(genes_histone_joined, vertices = complex_vertices, directed = FALSE)
```

The below figure is the network developed from the data and enriched gene set. Proteins that could be assigned to known complexes are colored in respectively.

```{r network-graph}
plot(complex_graph, edge.color = "black", vertex.label.color = "black", vertex.size = 8, layout = layout_nicely, rescale = TRUE, asp = 0)
title("Figure 6c: Network visualization")
```

This legend shows all identified complexes and their colors, whether or not they made it into the network visualization.

```{r network-legend}
plot.new()
legend(x = "center", legend = unique(genes_histone_joined$Complex), fill = unique(genes_histone_joined$color), ncol = 3, cex = 0.75)
title("Figure 6d: Legend")
```
...
ggplot(sig_network, aes(x = Complex, y = weight)) +
  ...
  geom_label(aes(label = Symbol, angle = 30), size = 3,
  ...
  labs(title = "Figure 6e: Network components")
```
...
ggplot(sig_network, aes(x = Complex, y = Symbol, fill = PTM)) +
  ...
  labs(title = "Figure 6f: Network bins")
```